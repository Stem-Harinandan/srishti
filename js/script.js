...
function skipPreloader() {
  if (!preloaderEl || preloaderEl.classList.contains("hidden")) return;
...
// Check if preloader has been seen before
const preloaderSeen = localStorage.getItem("preloaderSeen") === "true";

if (preloaderSeen && preloaderEl) {
  ...
  // Show preloader
  preloaderEl.addEventListener("click", skipPreloader);
  preloaderEl.addEventListener("keydown", (event) => {
...
if (introVideo) {
  introVideo.addEventListener("ended", skipPreloader);
  introVideo.addEventListener("error", skipPreloader);
}
function getScrollY() {
...
if (!isTouchDevice) {
  ...
  // Move cursor dot with mouse (no delay)
  document.addEventListener("mousemove", (e) => {
    ...
    // Move cursor ring with a slight delay
    if (cursorRing) {
      const ringX = parseFloat(cursorRing.style.left) || centerX;
      const ringY = parseFloat(cursorRing.style.top) || centerY;
      const newRingX = ringX + (e.clientX - ringX) * 0.1;
      const newRingY = ringY + (e.clientY - ringY) * 0.1;
      cursorRing.style.left = `${newRingX}px`;
      cursorRing.style.top = `${newRingY}px`;
    }
  });
}
